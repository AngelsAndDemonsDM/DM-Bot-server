...
class DownloadServerModule(Server):
    async def download_server_conent(self, user_login):
        zip_path = self._create_zip_archive()
        await self.send_file_login(user_login, zip_path, 'server_contet.zip')
    ...
    def _create_zip_archive(self) -> Path:
        ...
        if archive_path.exists():
            if archive_path.stat().st_mtime >= self._get_latest_modification_time(folder_path):
                return archive_path
